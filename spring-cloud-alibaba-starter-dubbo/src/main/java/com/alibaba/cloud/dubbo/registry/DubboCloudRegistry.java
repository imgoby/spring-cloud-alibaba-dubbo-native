...
import com.alibaba.cloud.dubbo.util.MetaUtil;
...
public class DubboCloudRegistry extends FailbackRegistry
	...
	private boolean instanceSame(ServiceInstance newInstance,
		...
		//if (!oldInstance.getMetadata().equals(newInstance.getMetadata())) { //by goby
		if (!MetaUtil.parseMeta(oldInstance).equals(MetaUtil.parseMeta(newInstance))) {
			return false;
		}
	...
	private boolean isDubboServiceInstance(ServiceInstance serviceInstance) {
		Map<String, String> metadata = MetaUtil.parseMeta(serviceInstance);//serviceInstance.getMetadata();
